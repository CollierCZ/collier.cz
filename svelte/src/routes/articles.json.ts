...
export function getArticlesJson(): ArticleMetadata[] {
  ...
  const filteredArticleKeys = articleKeys.filter(
    (article) =>
      new Date(allArticles[article].metadata.published) <= new Date(),
  )

  return filteredArticleKeys.reduce(
    (articlesMetadataCollector, article): Array<ArticleMetadata> => {
      const articleMetadata = allArticles[article].metadata
      const slug = article
        .replace('/src/routes/articles/', '')
        .replace('.svelte.md', '')
      const { category, description, heroImage, published, tags, title } =
        articleMetadata
      return [
        ...articlesMetadataCollector,
        { category, description, heroImage, published, slug, tags, title },
      ]
    },
    [],
  )
